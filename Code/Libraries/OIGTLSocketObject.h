 ...
 * 
 * OIGTLSocketObject implements the fuctionality required to send and receive messages using the OpenIGTLink 2.0 protocol, in a multi-threaded manner.
 * Each OIGTLSocketObject contains a sender and a receiver thread which share a common mutex. A socket object can be requested to connect to a remote host,
 * or to act as a server by listening on a local port. The current connection status is reported by emitting Qt signals. When connected, messages can be sent
 * through the socket and a signal is emitted (messageReceived(..)) when new data has arrived from the remote peer.
 * Connection status can also be queried via member functions (isListening(), isConnected()... ).
 */
